...
/**
 * Represents a block of flats with 8 stories and 3 apartments per floor with approximately 60 people living there.
 ...
 */
public class BlockOfFlats implements Building {
    ...
    private double energyConsumption = random.nextInt(200_000, 350_000);
    ...
    /**
     * Constructor to initialize an BlockOfFlats with a given kind of building.
     * @param kind The type of the building influencing budget and emissions.
     * PRE: kind != null
     * POST: A new BlockOfFlats object is initialized with associated materials and metrics.
     */
    // BAD OOP: high coupling due to its reliance on KindOfBuilding and direct material instantiations.
    public BlockOfFlats(KindOfBuilding kind) {
        this.kind = kind;
        shellMaterial = new ShellMaterial(kind.getShellMaterialCoeff(), random.nextInt(1200,2000));
        finishMaterial = new FinishMaterial(kind.getFinishMaterialCoeff(), random.nextInt(200,400));
        ...
        budget = (shellMaterial.getInitialCosts() + finishMaterial.getInitialCosts()) * kind.getEmissionCoeff();
