...
/**
 * Represents an academic building such as a university or high school.
 * Implements the Building interface, providing methods for managing
 * building lifecycle, budget, sustainability, and other metrics.
 */
public class AcademicalBuilding implements Building {
    ...
    private double energyConsumption = random.nextInt(200000, 400000);
    ...
    /**
     * Constructor to initialize an AcademicalBuilding with a given kind of building.
     * @param kind The type of the building influencing budget and emissions.
     * PRE: kind != null
     * POST: A new AcademicalBuilding object is initialized with associated materials and metrics.
     */
    public AcademicalBuilding(KindOfBuilding kind) {
        this.kind = kind;
        shellMaterial = new ShellMaterial(kind.getShellMaterialCoeff(), random.nextInt(1400, 2700));
        finishMaterial = new FinishMaterial(kind.getFinishMaterialCoeff(), random.nextInt(320, 720));
    ...
    public double getEmissionsPerPersonPerYear(int residents) {
        return (emissions * 0.001) / residents / age;
    }
     ...
     */
    // BAD OOP: long and contains multiple responsibilities - not just for repairing the building,
    // but also for updating the satisfaction index, recalculating emissions, and adjusting the budget.
    @Override
